...
/**
 * A class used to log different strings in a specific configuration. The
 * configuration has a name and different fields, which are logged using a log
 * table. The configuration can take the field `multiple`. If this field is set
 * to true, each field can have multiple entries. If the config has a format
...
class SbLogField extends SbLogTable {
  ...
  /**
   * Instantiates a new `SbLogField` object with the appropriate config.
